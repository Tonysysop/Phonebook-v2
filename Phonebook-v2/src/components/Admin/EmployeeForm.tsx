...
export const EmployeeForm: React.FC<EmployeeFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = "Name/office name is required";
    }

    // Email is optional, but validate format if user enters one
    if (
      formData.email.trim() &&
    ...
    // Role is optional, so no "required" check
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.extension && (
                <p className="text-red-600 text-xs mt-1">{errors.extension}</p>
              )}
            </div>
            <div>
              <label
